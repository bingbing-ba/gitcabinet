...
const unstaging = new PlainFile('unstaging.txt', basicAdd.refDirectory)
unstaging.content = '이 파일은 staging 하지 않아요.'
// 문제에 맞게 git add 만 하면 되도록 미리 git init해둠
basicAdd.setGit()
basicAdd.setBase()
basicAdd.setAnswer((_, git) => {
  if (git) {
...
basicAdd.setDefaultQueue([visualizationArea.GitStagingArea, visualizationArea.GitDirectory])
