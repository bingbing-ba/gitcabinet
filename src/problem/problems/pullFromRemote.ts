...
export const pullFromRemote = new Problem('원격 저장소에서 pull하기')

pullFromRemote.content = `현재 이 저장소에는 *README.md* 를 기록한 하나의 commit 만이 존재합니다. 원격 저장소에는 해당 commit 이후에 *pull.txt* 를 기록한 또다른 commit이 존재합니다. 즉 두 개의 commit이 존재합니다.
원격 저장소의 commit을 이 저장소로 pull 해주세요!
해당 원격 저장소는 이 저장소에 *origin* 으로 등록되어 있습니다.`
pullFromRemote.explanation = `원격 저장소의 내용을 내 컴퓨터의 저장소에 내려받는 것을 *pull* 이라고 합니다. *clone* 과 다른 점은, *clone* 은 내 컴퓨터에 저장소가 존재하지 않고, *pull* 은 저장소가 존재한다는 점 입니다. 덧붙이면,
...
pullFromRemote.setGit()
pullFromRemote.git?.setUserConfig({ name: 'bing', type: 'name' })
pullFromRemote.git?.setUserConfig({
  email: 'bing@bing.com',
  type: 'email',
})
const README = new PlainFile('README.md', pullFromRemote.refDirectory)
README.content = 'README.md 파일'
pullFromRemote.git?.add()
pullFromRemote.git?.commit('README.md 파일 추가')
...
pullFromRemote.git?.addRemote('origin', origin)
pullFromRemote.git?.push('origin', 'master')
const pull = new PlainFile('pull.txt', origin.refDirectory)
pull.content = 'pull.txt 파일'
origin.add()
const { hash } = origin.commit('pull.txt 파일 추가')
pullFromRemote.setBase()
pullFromRemote.setAnswer((_, git) => {
  if (git) {
    return git.branches[git.head] === hash
  }
  return false
})
pullFromRemote.setDefaultQueue([visualizationArea.GitRemote, visualizationArea.GitGraph])
